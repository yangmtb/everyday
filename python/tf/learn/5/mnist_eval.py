...
def evaluate(mnist):
    with tf.Graph().as_default() as g:
        ...
        # 通过变量重命名的方式来加载模型，这样在前向传播的过程中就不需要调用求滑动平均的函数来获取平均值了。这样就可以完全共用 mnist_inference.py 中定义的前向传播过程
        ...
        while True:
            with tf.Session() as sess:
                ...
                if ckpt and ckpt.model_checkpoint_path:
                    ...
                    print("AAfter %s training step(s), validation accuracy = %g" % (global_step, accuracy_score))
                else:
