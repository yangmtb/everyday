...
int main(int argc, char* argv[])
{
	...
	unsigned char *out_buffer=(unsigned char *)av_malloc(avpicture_get_size(AV_PIX_FMT_YUV420P, pCodecPar->width, pCodecPar->height));
	//avpicture_fill((AVPicture *)pFrame, out_buffer, AV_PIX_FMT_YUV420P, pCodecCtx->width, pCodecCtx->height);
	avpicture_fill((AVPicture *)pFrameYUV, out_buffer, AV_PIX_FMT_YUV420P, pCodecPar->width, pCodecPar->height);
	//SDL----------------------------
	...
	SDL_Window *screen;
	screen = SDL_CreateWindow("sdl2", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, screen_w, screen_h, SDL_WINDOW_OPENGL|SDL_WINDOW_RESIZABLE);
	...
	SDL_Texture *texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING, screen_w, screen_h);
	//SDL End------------------------
	int ret, got_picture;
	...
	img_convert_ctx = sws_getContext(pCodecPar->width, pCodecPar->height, (AVPixelFormat)pCodecPar->format, pCodecPar->width, pCodecPar->height, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL);
	//------------------------------
		...
		if (event.type == SFM_REFRESH_EVENT) {
			...
			if (av_read_frame(pFormatCtx, packet) >= 0) {
				if (packet->stream_index == videoindex) {
					...
					if (got_picture) {
						sws_scale(img_convert_ctx, (const unsigned char* const*)pFrame->data, pFrame->linesize, 0, pCodecPar->height, pFrameYUV->data, pFrameYUV->linesize);
