...
map<int, WebsocketHandler *> wsMap;
...
void handleRead(int efd, int fd) {
  wsMap.try_emplace(fd, new WebsocketHandler(fd));
  WebsocketHandler *ws = wsMap.at(fd);
  ws->mReaded = "";
  char buf[4096];
  int n = 0;
  //int off = 0;
  //Websocket_Handler *handler = wsMap[fd];
  while ((n = ::read(fd, buf, sizeof(buf))) > 0) {
    if (output_log)
      printf("read %d bytes\n", n);
    string &readed = ws->mReaded;
    readed.append(buf, n);
    //memcpy(handler->getbuff()+off, buf, n);
    //off += n;
    if (readed.length() > 4) {
      if (readed.substr(readed.length() - 2, 2) == "\n\n" || readed.substr(readed.length() - 4, 4) == "\r\n\r\n") {
        //当读取到一个完整的http请求，测试发送响应
        cout << "have a http request : \n" << readed << endl;
        updateEvents(efd, fd, EPOLLOUT | EPOLLET, EPOLL_CTL_MOD);
        //sendRes(fd);
      }
    }
  }
...
void loop_once(int efd, int lfd, int waitms) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (events & (EPOLLIN | EPOLLERR)) {
      if (fd == lfd) {
        //cout << "================== accept: " << fd << endl;
        executor.Commit(handleAccept, efd, fd);
        //handleAccept(efd, fd);
      } else {
        cout << "=================== read: " << fd << endl;
      ...
      cout << "==================== write: " << fd << endl;
      executor.Commit(handleWrite, efd, fd);
      //handleWrite(efd, fd);
    } else {
